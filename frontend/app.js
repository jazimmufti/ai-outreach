...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Default assumed connected account for the user (@ummer.04 for IG, 151600386127968346 for Discord)
    // Allows testing unlinked cases via ?unlinked=true, ?unlinked_discord=true, or localStorage
    try {
        ...
        const forceUnlinkedDiscord = urlParams.get("unlinked_discord") === "true" || urlParams.get("unlinked_discord") === "1" || urlParams.get("discord_connected") === "false";
        const storedDiscord = localStorage.getItem("arclent_linked_discord");
        if (forceUnlinkedDiscord || storedDiscord === "unlinked" || storedDiscord === "none") {
    ...
    const discordIdBannerTitle = document.getElementById("discord-id-banner-title");
    const discordServerExplanation = document.getElementById("discord-server-explanation");
    const btnDiscordVisitServer = document.getElementById("btn-discord-visit-server");
    const discordIdTip = document.getElementById("discord-id-tip");
    ...
    // Modal Elements for Connecting Discord (when description credits are detected)
    const modalConnectDiscord = document.getElementById("modal-connect-discord");
    const btnModalCdClose = document.getElementById("btn-modal-cd-close");
    const modalCdTitle = document.getElementById("modal-cd-title");
    const modalCdDesc = document.getElementById("modal-cd-desc");
    const modalDetectedDiscordList = document.getElementById("modal-detected-discord-list");
    const modalCdChipsContainer = document.getElementById("modal-cd-chips-container");
    const modalCdChipsRow = document.getElementById("modal-cd-chips-row");
    const modalCdForm = document.getElementById("modal-cd-form");
    const modalCdInput = document.getElementById("modal-cd-input");
    const modalCdErr = document.getElementById("modal-cd-err");
    const btnModalCdSubmit = document.getElementById("btn-modal-cd-submit");
    const btnModalCdSkip = document.getElementById("btn-modal-cd-skip");

    let currentModalExtractedDiscord = [];

    function showConnectDiscordModal(accounts = [], discoveryData = null) {
        if (!modalConnectDiscord) return;
        ...
        // CRITICAL: Must ONLY be there if user has NOT connected Discord
        if (state.linkedDiscordAccount) {
            return;
        }

        const rawList = Array.isArray(accounts) ? accounts : [];
        const cleanList = rawList.filter(a => a && typeof a === "string" && a.trim().length > 0);
        if (cleanList.length === 0) {
            return;
        }

        currentModalExtractedDiscord = cleanList;
        const formatted = currentModalExtractedDiscord.map(a => {
        ...
        if (modalDetectedDiscordList) modalDetectedDiscordList.textContent = formatted;
        if (modalCdTitle) modalCdTitle.textContent = "Credits are already mentioned!";
        if (modalCdDesc) {
            const roleStr = state.userRole ? `for ${escapeHtml(state.userRole.toLowerCase())} ` : "";
            modalCdDesc.innerHTML = `Credits ${roleStr}<strong>${escapeHtml(formatted)}</strong> were found in the video description. To get <strong>auto-verified</strong>, connect your Discord account.`;
        }

        if (modalCdChipsRow && modalCdChipsContainer) {
            modalCdChipsRow.innerHTML = "";
            currentModalExtractedDiscord.forEach(a => {
                const trimmed = (a || "").trim();
                const isSnowflake = /^\d{17,20}$/.test(trimmed);
                const chipLabel = isSnowflake ? `ID: ${trimmed}` : (trimmed.startsWith("@") ? trimmed : `@${trimmed}`);
                ...
                chip.addEventListener("click", () => {
                    if (modalCdInput) {
                        modalCdInput.value = cleanValue;
                        modalCdInput.focus();
                    }
                    modalCdChipsRow.querySelectorAll(".modal-chip-btn").forEach(c => c.classList.remove("selected"));
                    chip.classList.add("selected");
                });
                modalCdChipsRow.appendChild(chip);
            });
            modalCdChipsContainer.classList.remove("hidden");
        }

        // Pre-fill input if single candidate
        if (currentModalExtractedDiscord.length === 1 && modalCdInput) {
            modalCdInput.value = currentModalExtractedDiscord[0].trim().replace(/^@+/, "");
        } else if (modalCdInput && !modalCdInput.value && state.linkedDiscordAccount) {
            modalCdInput.value = state.linkedDiscordAccount;
        }

        if (modalCdErr) modalCdErr.classList.add("hidden");
        modalConnectDiscord.classList.remove("hidden");
        setTimeout(() => {
            if (modalCdInput) modalCdInput.focus();
        }, 60);
    }

    function hideConnectDiscordModal() {
        if (modalConnectDiscord) {
            modalConnectDiscord.classList.add("hidden");
        }
    }

    if (modalCdForm) {
        modalCdForm.addEventListener("submit", async (e) => {
            e.preventDefault();
            const rawVal = modalCdInput ? modalCdInput.value.trim() : "";
            const cleanAccount = rawVal.replace(/^@+/, "").trim();

            if (!cleanAccount) {
                if (modalCdErr) {
                    modalCdErr.textContent = "Please enter your Discord User ID or username";
                    modalCdErr.classList.remove("hidden");
                }
                return;
            }

            state.linkedDiscordAccount = cleanAccount;
            try {
                localStorage.setItem("arclent_linked_discord", cleanAccount);
            } catch (_) {}
            saveSessionState();

            if (btnModalCdSubmit) {
                btnModalCdSubmit.disabled = true;
                btnModalCdSubmit.innerHTML = `<span class="analyzing-spinner" style="width: 14px; height: 14px; display: inline-block; vertical-align: middle; margin-right: 6px;"></span> Verifying...`;
            }

            let verifyData = null;
            if (state.sessionId) {
                try {
                    const res = await fetch("/api/outreach/verify-linked-discord-account", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({
                            session_id: state.sessionId,
                            discord_account: cleanAccount
                        })
                    });
                    if (res.ok) {
                        verifyData = await res.json();
            ...
            if (verifyData && verifyData.verified) {
                hideConnectDiscordModal();
                renderAutoVerifiedSuccess(verifyData.auto_verification || { matched_account: cleanAccount, platform: "Discord" });
                return;
            }

            hideConnectDiscordModal();
            showToast(`Connected Discord ID ${cleanAccount}. Proceeding to outreach.`, "info");
        });
    }

    if (btnModalCdClose) {
        btnModalCdClose.addEventListener("click", () => {
            hideConnectDiscordModal();
        });
    }

    if (btnModalCdSkip) {
        btnModalCdSkip.addEventListener("click", () => {
            hideConnectDiscordModal();
        });
    }

    if (modalConnectDiscord) {
        modalConnectDiscord.addEventListener("click", (e) => {
            if (e.target === modalConnectDiscord) {
                hideConnectDiscordModal();
            }
        });
    }
    ...
    function handleDiscoveryCompleted(data) {
        ...
        const isUnlinkedIg = !state.linkedInstagramAccount;
        const isUnlinkedDiscord = !state.linkedDiscordAccount;
        ...
        if (isUnlinkedDiscord && isDiscordCredit && extracted.length > 0) {
            state.stage = "verify_email";
            renderVerifyEmailStep();
            showScreen("verifyEmail", 3);
            showConnectDiscordModal(extracted, data);
            return;
        }

        // CASE 2B: Credits are already mentioned in the description and user hasn't linked Instagram
        if (isUnlinkedIg && extracted.length > 0 && !isDiscordCredit) {
            state.stage = "verify_email";
            renderVerifyEmailStep();
            showScreen("verifyEmail", 3);
        ...
        // CASE 3: No credits mentioned or not verified -> keep connect modals & banner hidden
    ...
    // Handle multi-platform extension outreach dispatch (Instagram, X, Discord, Facebook)
    async function dispatchSocialWithExtension({ platform = "instagram", username, userId, message, sessionId }) {
        const p = (platform || "instagram").toLowerCase();
        let cleanUsername = username;
        if (p.includes("instagram") || p === "ig") {
            cleanUsername = normalizeInstagramHandle(username);
        } else if (p.includes("x") || p.includes("twitter")) {
            cleanUsername = String(username || "").replace(/^https?:\/\/(www\.)?(x\.com|twitter\.com)\//i, "").split("?")[0].replace(/\/+$/, "").replace(/^@+/, "").trim();
        } else if (p.includes("facebook") || p.includes("messenger")) {
            cleanUsername = String(username || "").replace(/^https?:\/\/(www\.)?(facebook\.com|m\.me|messenger\.com)\//i, "").split("?")[0].replace(/\/+$/, "").replace(/^@+/, "").trim();
        } else if (p.includes("discord")) {
            cleanUsername = userId || username;
        }

        const c = state.creator || {};
        const creatorName = c.name || c.channel_name || "Creator";
        const meta = getSocialMediaMeta(p);
        const handle = formatHandle(cleanUsername || userId || meta.name);
        ...
        // Post outreach message to extension via bridge (sends both universal and legacy type)
        window.postMessage({
            type: "ARCLENT_SOCIAL_OUTREACH",
            platform: p,
            username: cleanUsername,
            userId: userId,
            message: message,
        ...
        showToast(`✓ Opening ${meta.name} DM for ${handle}... Review & click Send!`);
    ...
    // Backwards compatibility alias for Instagram outreach
    async function dispatchInstagramWithExtension({ username, message, sessionId }) {
        return dispatchSocialWithExtension({
            platform: "instagram",
            username: username,
            message: message,
            sessionId: sessionId
    ...
    async function dispatchSocialOutreach(options = {}) {
        ...
        // Accurately resolve handle: options.handle -> platform-specific manual/confirmed -> active.username
        let rawHandle = options.handle;
        if (!rawHandle) {
            if (pLower.includes("instagram") || pLower === "ig") {
                rawHandle = state.finalInstagramHandle || active.username;
            } else if (pLower.includes("x") || pLower.includes("twitter")) {
                rawHandle = state.finalXHandle || active.username;
            } else if (pLower.includes("facebook") || pLower === "fb") {
                rawHandle = state.finalFacebookHandle || active.username;
            } else if (pLower.includes("discord")) {
                rawHandle = state.finalDiscordUserId || active.discord_user_id || active.username;
            } else {
                rawHandle = active.username;
            }
        }
        const handle = formatHandle(rawHandle || platformName);
        const c = state.creator || {};
        const creatorName = c.name || c.channel_name || "Creator";
        ...
        const clickedBtn = options.button || 
            (btnSendDiscordBot && !btnSendDiscordBot.closest(".hidden") ? btnSendDiscordBot :
        ...
        if (platformName.toLowerCase() === "discord") {
            ...
            // Direct messages cannot be sent to a server itself; require creator's Discord User ID
            if (!discUserId) {
                if (hasServerInvite) {
                    showToast("Discord server detected. Direct messaging requires the creator's 17-20 digit Discord User ID.", "info");
                } else {
            ...
            // If extension is installed on desktop, use the extension to open Discord DM and prefill
            const isDesktop = !isMobileDevice();
            const extensionAvailable = isDesktop && (checkDirectDomPresence() || !!state.extensionInstalled);
            if (extensionAvailable) {
                await dispatchSocialWithExtension({
                    platform: "discord",
                    username: discUserId,
                    userId: discUserId,
                    message: text,
            ...
            const executeDiscordOpen = async () => {
                ...
                if (state.sessionId) {
                    const senderIdentity = state.linkedDiscordAccount ? `Discord User ${state.linkedDiscordAccount}` : "Someone on Arclent";
                    fetch("/api/outreach/record-social-outreach", {
                        method: "POST",
                        headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({
                            session_id: state.sessionId,
                            platform: "Discord",
                            handle: discUserId || "Discord Server",
                            sender_handle: state.linkedDiscordAccount || null,
                            sender_identity: senderIdentity,
                            message: text
                        })
                    }).catch(() => {});
        ...
        const isInstagram = pLower.includes("instagram") || pLower === "ig";
        const isDesktop = !isMobileDevice();
        const isSupportedByExtension = ["instagram", "ig", "x", "twitter", "facebook", "messenger", "fb"].some(s => pLower.includes(s));
        const extensionInstalled = isDesktop && isSupportedByExtension && (checkDirectDomPresence() || !!state.extensionInstalled);
        ...
        let targetHandleOrUrl = options.url;
        if (!targetHandleOrUrl) {
            if (active.url && active.username && formatHandle(active.username) === handle) {
                targetHandleOrUrl = active.url;
            } else {
                targetHandleOrUrl = handle;
            }
        }
    ...
    function isProfileUserProvided(p) {
        ...
        if (pLower.includes('x') || pLower.includes('twitter')) {
            if (state.xProfile && (state.xProfile.source === 'Manual entry' || state.xProfile.user_provided)) {
                return true;
            }
        }
        if (pLower.includes('facebook') || pLower === 'fb') {
            if (state.facebookProfile && (state.facebookProfile.source === 'Manual entry' || state.facebookProfile.user_provided)) {
                return true;
            }
        }
        if (pLower.includes('instagram') || pLower === 'ig') {
    ...
    function updateStep2PlatformUI(profile) {
        ...
        const isDiscordServer = isDiscord && !snowflake && Boolean(
            p.discord_invite ||
            (p.url && (p.url.includes("discord.gg") || p.url.includes("discord.com/invite") || p.url.includes("discord.io") || p.url.includes("discord.me") || p.url.includes("/servers/"))) ||
            (p.username && (p.username.includes("discord.gg") || p.username.includes("discord.com/invite") || p.username.includes("discord.io") || p.username.includes("discord.me")))
        );
        const serverInviteUrl = isDiscordServer ? (p.discord_invite || p.url || (p.username && p.username.startsWith("http") ? p.username : `https://${p.username}`)) : null;
        ...
        if (isDiscord && snowflake) {
            ...
            handle = m ? `discord.gg/${m[1]}` : (serverInviteUrl || "Discord Server").replace(/^https?:\/\//i, '');
        } else {
            handle = formatHandle(p.username || platformKey);
        }
        const cleanHandle = snowflake || handle.replace('@', '');
        const url = isDiscordServer 
            ? (serverInviteUrl || "https://discord.com") 
            : (p.url || (snowflake ? `https://discord.com/users/${snowflake}` : `${defaultBase}/${cleanHandle}`));
        ...
        if (igStepHeading) {
            igStepHeading.textContent = isDiscordServer ? "Discord Server Detected" : `Confirm ${meta.name} Profile`;
        }
        if (igStepSubheading) {
            if (isDiscordServer) {
                igStepSubheading.innerHTML = `We found a Discord community server for <strong id="ig-found-creator-name">${escapeHtml(creatorName)}</strong>.`;
        ...
        if (igProfileBadge) {
            if (isDiscordServer) {
                igProfileBadge.textContent = "DISCORD SERVER";
                igProfileBadge.style.background = "#EEF2FF";
                igProfileBadge.style.color = "#4338CA";
                igProfileBadge.style.borderColor = "#6366F1";
            } else {
                igProfileBadge.textContent = isUserProvided ? "PROVIDED PROFILE" : "DETECTED PROFILE";
                igProfileBadge.style.background = "";
                igProfileBadge.style.color = "";
                igProfileBadge.style.borderColor = "";
            }
        }
        ...
        if (igDiscordNotice) {
            if (isDiscordServer) {
                ...
                igDiscordNotice.innerHTML = `📢 <strong>Discord Server Detected:</strong> This is a community server, not a direct user account. Discord servers cannot receive direct messages. To message <strong>${escapeHtml(creatorName)}</strong> directly, you will need their <strong>17-20 digit Discord User ID</strong>. You can visit the server below, right-click the creator's avatar/name in the member list, select <em>Copy User ID</em>, and paste it.`;
            } else if (isDiscord) {
                igDiscordNotice.classList.remove("hidden");
    ...
    function renderDiscoveredOtherSocials() {
        ...
        platformsConfig.forEach(cfg => {
            ...
            if (isDetected) {
                ...
                let displayHandle;
                let isDiscordServerProfile = false;
                if (cfg.key === "discord") {
                    if (isNumericId || detectedProfile.discord_user_id) {
                        ...
                        isDiscordServerProfile = true;
                        const m = detectedProfile.discord_invite.match(/discord(?:\.gg|\.com\/invite|\.io|\.me)\/([a-zA-Z0-9_\-]+)/i);
                        displayHandle = m ? `Server: discord.gg/${m[1]}` : `Server: ${detectedProfile.discord_invite.replace(/^https?:\/\//i, '')}`;
                        ...
                        displayHandle = m ? `Server: discord.gg/${m[1]}` : `Server: ${detectedProfile.url.replace(/^https?:\/\//i, '')}`;
    ...
    function selectActiveSocialProfile(social) {
        ...
        if (pLower.includes("discord")) {
        ...
        } else if (pLower === "x" || pLower.includes("twitter")) {
            state.xProfile = {
                platform: "X",
                username: cleanHandle,
                url: url
            };
            state.finalXHandle = cleanHandle;
            state.finalXUrl = url;
        } else if (pLower.includes("facebook") || pLower === "fb") {
            ...
            state.finalFacebookHandle = cleanHandle;
            state.finalFacebookUrl = url;
        }
    ...
    function renderVerifyInstagramStep() {
        ...
        if (state.selectedChannel && state.socialProfiles && state.socialProfiles.length > 0) {
            const matchedProfile = state.socialProfiles.find(s => (s.platform || "").toLowerCase() === state.selectedChannel.toLowerCase());
            if (matchedProfile) {
                state.activeSocialProfile = matchedProfile;
            }
        }
        ...
        if (isDiscordServer) {
            ...
            if (state.instagramConfirmed) {
                let confirmedHandle = active.username;
                let confirmedUrl = active.url;

                if (pLower.includes("instagram") || pLower === "ig") {
                    confirmedHandle = state.finalInstagramHandle || active.username;
                    confirmedUrl = state.finalInstagramUrl || active.url;
                } else if (pLower === "x" || pLower.includes("twitter")) {
                    confirmedHandle = state.finalXHandle || active.username;
                    confirmedUrl = state.finalXUrl || active.url;
                } else if (pLower.includes("facebook") || pLower === "fb") {
                    confirmedHandle = state.finalFacebookHandle || active.username;
                    confirmedUrl = state.finalFacebookUrl || active.url;
                } else if (pLower.includes("discord")) {
                    const discId = state.finalDiscordUserId || snowflake || active.username;
                    confirmedHandle = discId && /^\d{17,20}$/.test(discId) ? `User ID: ${discId}` : discId;
                    confirmedUrl = discId && /^\d{17,20}$/.test(discId) ? `https://discord.com/users/${discId}` : active.url;
                }
    ...
    function showInstagramConfirmed(handle, url) {
        ...
        if (state.activeSocialProfile) {
            state.activeSocialProfile.username = handle;
            state.activeSocialProfile.url = url;
            state.activeSocialProfile.user_provided = true;
        }
        if (state.selectedSocialProfile) {
            state.selectedSocialProfile.username = handle;
            state.selectedSocialProfile.url = url;
            state.selectedSocialProfile.user_provided = true;
        }
        ...
        if (pLower.includes("instagram") || pLower === "ig") {
            state.finalInstagramHandle = handle;
            state.finalInstagramUrl = url || `https://instagram.com/${handle.replace(/^@+/, '')}`;
            state.instagramProfile = {
                platform: "Instagram",
                username: handle,
                url: state.finalInstagramUrl,
            ...
            state.finalXHandle = handle;
            state.finalXUrl = url || `https://x.com/${handle.replace(/^@+/, '')}`;
        } else if (pLower.includes("facebook") || pLower === "fb") {
            state.finalFacebookHandle = handle;
            state.finalFacebookUrl = url;
        } else if (pLower.includes("discord")) {
            const sn = extractDiscordSnowflake(handle);
            if (sn) state.finalDiscordUserId = sn;
        }
    ...
    if (igManualEntryForm) {
        igManualEntryForm.addEventListener("submit", async (e) => {
            ...
            try {
                ...
                // Update active profile object immediately
                active.username = formatted;
                active.url = url;
                active.handle = formatted;
                active.user_provided = true;
                active.source = "Manual entry";

                if (state.activeSocialProfile) {
                    state.activeSocialProfile.username = formatted;
                    state.activeSocialProfile.url = url;
                    state.activeSocialProfile.user_provided = true;
                    state.activeSocialProfile.source = "Manual entry";
                }
                if (state.selectedSocialProfile) {
                    state.selectedSocialProfile.username = formatted;
                    state.selectedSocialProfile.url = url;
                    state.selectedSocialProfile.user_provided = true;
                    state.selectedSocialProfile.source = "Manual entry";
                }
                ...
                if (pLower.includes("instagram") || pLower === "ig") {
                    ...
                    state.finalFacebookHandle = formatted;
                    state.finalFacebookUrl = url;
                    state.facebookProfile = {
                        platform: "Facebook",
                        username: formatted,
                ...
                // Update state.socialProfiles so grid and future selections use this manual handle
                if (!state.socialProfiles) state.socialProfiles = [];
                const sIdx = state.socialProfiles.findIndex(s => (s.platform || "").toLowerCase() === (active.platform || "").toLowerCase());
                const manualSocialObj = {
                ...
                if (sIdx >= 0) {
                    state.socialProfiles[sIdx] = manualSocialObj;
                } else {
                    state.socialProfiles.push(manualSocialObj);
                }
    ...
    if (btnIgOpenSend) {
        btnIgOpenSend.onclick = () => {
            const active = state.activeSocialProfile || state.selectedSocialProfile || state.instagramProfile || {};
            const pName = active.platform || "Instagram";
            const pLower = pName.toLowerCase();
            let curHandle = active.username;
            let curUrl = active.url;

            if (pLower.includes("instagram") || pLower === "ig") {
                curHandle = state.finalInstagramHandle || active.username;
                curUrl = state.finalInstagramUrl || active.url;
            } else if (pLower.includes("x") || pLower.includes("twitter")) {
                curHandle = state.finalXHandle || active.username;
                curUrl = state.finalXUrl || active.url;
            } else if (pLower.includes("facebook") || pLower === "fb") {
                curHandle = state.finalFacebookHandle || active.username;
                curUrl = state.finalFacebookUrl || active.url;
            } else if (pLower.includes("discord")) {
                curHandle = state.finalDiscordUserId || active.username;
                curUrl = active.url;
            }
    ...
    if (btnBackIg) {
        btnBackIg.onclick = () => {
            ...
            // If on confirmed view, return to Step 2 manual fallback or detected view
            if (igConfirmedView && !igConfirmedView.classList.contains("hidden")) {
                if (isDiscordServer) {
                    showInstagramFallback(false, true);
                    return;
                }
                const isUserProvided = isProfileUserProvided(active);
                if (isUserProvided) {
                    showInstagramFallback(false);
                    return;
                }
    ...
    function renderOutreachHub() {
        ...
        const isDiscordServer = Boolean(
            disc && !discUserId && (
                disc.discord_invite ||
                (disc.url && (disc.url.includes("discord.gg") || disc.url.includes("discord.com/invite") || disc.url.includes("discord.io") || disc.url.includes("discord.me") || disc.url.includes("/servers/"))) ||
                (disc.username && (disc.username.includes("discord.gg") || disc.username.includes("discord.com/invite") || disc.username.includes("discord.io") || disc.username.includes("discord.me")))
            )
        );
        const serverInviteUrl = isDiscordServer ? (disc.discord_invite || disc.url || (disc.username && disc.username.startsWith("http") ? disc.username : `https://${disc.username}`)) : null;
        ...
        if (hubSummaryDiscordVal) {
            if (discUserId) {
                ...
                const m = (serverInviteUrl || "").match(/discord(?:\.gg|\.com\/invite|\.io|\.me)\/([a-zA-Z0-9_\-]+)/i);
                const shortServer = m ? `discord.gg/${m[1]}` : (serverInviteUrl || "Server").replace(/^https?:\/\//i, '');
                hubSummaryDiscordVal.textContent = `Server: ${shortServer} (User ID needed)`;
                hubSummaryDiscordVal.style.color = "var(--amber)";
        ...
        if (isSendable) {
            ...
            if (discordIdentificationBanner) {
                ...
                if (btnDiscordVisitServer && serverInviteUrl) {
                    ...
                    btnDiscordVisitServer.classList.remove("hidden");
                }
                if (discordIdTip) {
                    discordIdTip.innerHTML = `💡 Need their User ID? Click 'Visit Server' above, right-click the creator's avatar or name in the server member list, select <em>Copy User ID</em>, and paste it here.`;
                }
            }
            if (btnSendDiscordBot) btnSendDiscordBot.disabled = false;
            ...
            if (discordIdentificationBanner) {
                ...
                if (discordServerExplanation) discordServerExplanation.classList.add("hidden");
                if (btnDiscordVisitServer) btnDiscordVisitServer.classList.add("hidden");
                const targetStr = disc.discord_username || disc.username || "Discord";
                if (discordDiscoveredTarget) discordDiscoveredTarget.textContent = targetStr;
                if (discordIdTip) {
                    discordIdTip.innerHTML = `💡 Right-click creator profile in Discord &rarr; <em>Copy User ID</em>`;
                }
            }
            if (btnSendDiscordBot) btnSendDiscordBot.disabled = false;
        ...
        // Prioritize confirmed/manual Instagram, X, and Facebook handles in otherSocials
        if (state.finalInstagramHandle) {
            const cleanIg = state.finalInstagramHandle.replace(/^@+/, '');
            const igIdx = otherSocials.findIndex(s => (s.platform || "").toLowerCase() === "instagram");
            const igObj = {
                platform: "Instagram",
                username: state.finalInstagramHandle,
                url: state.finalInstagramUrl || `https://instagram.com/${cleanIg}`,
                source: "Confirmed profile",
                confidence: "high",
                user_provided: true
            };
            if (igIdx >= 0) {
                otherSocials[igIdx] = igObj;
            } else {
                otherSocials.unshift(igObj);
            }
        }

        if (state.finalXHandle) {
            const cleanX = state.finalXHandle.replace(/^@+/, '');
            const xIdx = otherSocials.findIndex(s => (s.platform || "").toLowerCase() === "x" || (s.platform || "").toLowerCase() === "twitter");
            const xObj = {
                platform: "X",
                username: state.finalXHandle,
                url: state.finalXUrl || `https://x.com/${cleanX}`,
                source: "Confirmed profile",
                confidence: "high",
                user_provided: true
            };
            if (xIdx >= 0) {
                otherSocials[xIdx] = xObj;
            } else {
                otherSocials.push(xObj);
            }
        }

        if (state.finalFacebookHandle) {
            const cleanFb = state.finalFacebookHandle.replace(/^@+/, '');
            const fbIdx = otherSocials.findIndex(s => (s.platform || "").toLowerCase() === "facebook" || (s.platform || "").toLowerCase() === "fb");
            const fbObj = {
                platform: "Facebook",
                username: state.finalFacebookHandle,
                url: state.finalFacebookUrl || `https://facebook.com/${cleanFb}`,
                source: "Confirmed profile",
                confidence: "high",
                user_provided: true
            };
            if (fbIdx >= 0) {
                otherSocials[fbIdx] = fbObj;
            } else {
                otherSocials.push(fbObj);
            }
        }
    ...
    async function sendDiscordOutreachMessage(opts = {}) {
        ...
        const inviteUrl = (state.discordProfile && (state.discordProfile.discord_invite || state.discordProfile.url));
        const hasServerInvite = inviteUrl && (inviteUrl.includes("discord.gg") || inviteUrl.includes("discord.com/invite") || inviteUrl.includes("discord.io") || inviteUrl.includes("discord.me"));

        if (!userId) {
            if (hasServerInvite) {
                showToast("A Discord server was detected. Please enter the creator's 17-20 digit Discord User ID to send outreach.", "info");
            } else {
        ...
        await dispatchSocialOutreach({
            platform: "Discord",
            handle: userId,
            text: text,
            button: btnSendDiscordBot,
            returnScreen: opts.returnScreen || "outreach_hub"
        });
    }
    ...
    const btnCopyDiscordDraftBtn = document.getElementById("btn-copy-discord-draft-btn");
    if (btnCopyDiscordDraftBtn) {
        btnCopyDiscordDraftBtn.onclick = async () => {
            const text = discordMessageBody ? discordMessageBody.value.trim() : "";
            if (!text) return;
            await copyTextToClipboard(text);
            const span = btnCopyDiscordDraftBtn.querySelector("span");
            if (span) {
                span.textContent = "✓ Message Copied!";
                setTimeout(() => { if (span) span.textContent = "📋 Copy Message"; }, 2500);
            }
            showToast("✓ Message copied to clipboard!");
        };
    }
    ...
    if (btnAddXManual && manualXInput) {
        const handleAddX = () => {
            ...
            if (!state.socialProfiles) state.socialProfiles = [];
            const existsIdx = state.socialProfiles.findIndex(s => (s.platform || "").toLowerCase() === "x" || (s.platform || "").toLowerCase() === "twitter");
            const newObj = {
                platform: "X",
                username: `@${raw}`,
                url: `https://x.com/${raw}`,
            ...
            if (existsIdx >= 0) {
                state.socialProfiles[existsIdx] = newObj;
            } else {
                state.socialProfiles.push(newObj);
            }
            state.finalXHandle = `@${raw}`;
            state.finalXUrl = `https://x.com/${raw}`;
            state.activeSocialProfile = newObj;
            saveSessionState();
            manualXInput.value = "";
    ...
    function renderAutoVerifiedSuccess(autoVerify) {
        ...
        const isDiscordMatch = autoVerify && (
            autoVerify.platform === "Discord" || 
            autoVerify.method === "youtube_description_discord_match" || 
            /^\d{17,20}$/.test(String(matchedAccount).trim())
        );
        const cleanHandle = String(matchedAccount).replace(/^@+/, "");
        ...
        showToast(isDiscordMatch ? "✓ Automatically verified! Your Discord account matches the video credits." : "✓ Automatically verified! Your username matches the video credits.");
    }
